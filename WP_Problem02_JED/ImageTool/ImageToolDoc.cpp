...
BEGIN_MESSAGE_MAP(CImageToolDoc, CDocument)
	...
	ON_COMMAND(ID_IMAGE_INVERSION, &CImageToolDoc::OnImageInversion)
...
void CImageToolDoc::OnImageInversion()
{
...
{
	// TODO: 여기에 명령 처리기 코드를 추가합니다.
	CBrightnessContrastControlDlg dlg;

	if (dlg.DoModal() == IDOK)
	{
		CONVERT_DIB_TO_BYTEIMAGE(m_Dib, img)
		IppBrightness(img, dlg.m_nBrightness);
		IppContrast(img, dlg.m_nContrast_2017136128);
		CONVERT_IMAGE_TO_DIB(img, dib)

		AfxPrintInfo(_T("[Brightness] Input Image: %s, Brightness : %d"),
			GetTitle(), dlg.m_nBrightness);
		AfxNewBitmap(dib);
	}
